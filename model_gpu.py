...
class GPTModel(nn.Module):
    ...
    def forward(self, X):
        ...
        X_pos = torch.arange(seq_len, device=X.device)
    ...
    def get_batch_loss(self, input_ids, target_ids):
        ...
        target_ids = target_ids.to(next(self.parameters()).device)
    ...
    def train_loop(self, train_dataloader, val_dataloader, optimizer, n_epochs):
        ...
        for epoch in range(n_epochs):
            ...
            for i, (input_batch, target_batch) in enumerate(train_dataloader):
                ...
                input_batch = input_batch.to(next(self.parameters()).device)
                target_batch = target_batch.to(next(self.parameters()).device)
